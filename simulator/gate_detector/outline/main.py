...
def gatenet():
    ...
    conv5 = conv2d_block(drop2, n_filters*4)
    ...
    up8 = Conv2DTranspose(n_filters*2, (3, 3), strides=(2,2), activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(drop5)
...
class ProgressImageSaver(keras.callbacks.Callback):
    ...
    def on_batch_end(self, batch, logs={}):
        if int(time.time()) > self.time + 5:
            ...
            self.imnm += 1
            utils.save_image(filename, image)
            self.time = int(time.time())
...
#utils.save_image("trainin.png", training_data[0][0])
#utils.save_image("trainout.png", training_data[1][0])
